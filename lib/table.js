...
Table.prototype.autoscale = function () {
  const self = this;

  return new Autoscaler({ table: self });
};
...
Table.prototype.awaitExists = function (callback) {
  const params = {
    TableName: this.tableName(),
  };

  this.docClient.service.waitFor('tableExists', params, callback);
};
